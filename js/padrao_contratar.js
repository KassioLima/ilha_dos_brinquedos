...
function total(confirmado, reservado)
{
	var data_evento = document.getElementsByName("data")[0].value;
	...
	if(data_evento != "")
	{
		...
		for(var x = 0; x < brinquedos.length; x++)
		{
			if (brinquedos[x].checked == true) 
			{
				...
				{
					if(data_reservada[i] == data_evento)
					{
				...
				{
					if(data_confirmada[i] == data_evento)
					{
						data_encontrada = true;
						mensagem = "Este já está confirmado para esta data";
						break;
					}
				}
